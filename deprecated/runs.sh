        ...
        for bounds in 10; do
            for envs in 4; do
                for type in 'MS'; do                                                                                                                            
                    ...
                    python genfranka.py -ne 128 -ni 1000 -f "${frequency}" -c -ti "${type}" -td 'train' -rm '5' -rcom '5' -rinr '5' -rstf '5' -rdam '5' -di -v -ds -dp -ri -rv -rcg
                    #python genfranka.py -ne 4 -ni 1000 -f "${frequency}" -c -ti "${type}" -td 'train' --sim_device 'cuda:0' -v -ds -dp
...
#                python generation_franka.py --frequency "${frequency}" --num-envs 32 --control-#imposed-file False --control-imposed True --osc-task False --type-of-input "${type}" --lower-#bound-mass "${bounds}" --higher-bound-mass "${bounds}" --save-tensors True --type-of-dataset #'train'
#            done 
#        done
#    done
#done
