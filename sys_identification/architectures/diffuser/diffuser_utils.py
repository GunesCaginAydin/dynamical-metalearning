...
def conditioning(x, condition=None, force=None, ctxlen=200, action_dim=7, condition_type=[False, False], to='train'):
    """
    Conditioning of sample/sampled data for contextual steering of trajectories,
	conditions are provided as dictionaries or masks as.
    """
    if to=='train': # training options to condition u | y seperately
        if force is not None and condition_type[0]: # condition u
            x[:, :, :action_dim] = force
	
        if condition is not None and condition_type[1]: # condition y
            x[:, :ctxlen, action_dim:] = condition
	
    elif to=='infer': # inference conditions are on u | y together
        x[:, :, :action_dim] = force
        x[:, :ctxlen, action_dim:] = condition
...
def create_timesteps(batch_size, i, device):
    """
	Batch x TimeStep for diffusion denoising.
    """
