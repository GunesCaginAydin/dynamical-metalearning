...
def naic(y_true, y_pred, time_axis=0, modelsize=0, datasize=0):
    ...
    Akaike Information Criterion is an estitor used in assessing the relative quality of
    ...
    """
    SSE = np.mean((y_pred - y_true)**2, axis=time_axis)
    aic = 2*modelsize - 2*np.log(SSE)
    caic = aic + 2*modelsize*(modelsize+1)/(datasize - modelsize - 1)
    return caic

def fpe(ytrue, y_pred, time_axis=0, modelsize=0, datasize=0):
    ...
    """
    RSS = np.sum(ytrue**2-y_pred**2, axis=time_axis)
    fpe = RSS/modelsize/(1 - datasize/modelsize)**2
    return fpe
