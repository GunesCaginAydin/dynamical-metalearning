...
class randomize():
    ...
    def com(self,nom,amount=0):
        ...
        if self.args.random_coms:
            lower_bound,higher_bound = self.decide_bounds(nom*(1-(amount)/100),nom*(1+(amount)/100))
    ...
    def inertia(self,nom,amount=0):
        ...
        if self.args.random_inertias:
            ...
            print('\n Inertia randomization:\nLower bound -->\n'+str(lower_bound.transpose().squeeze().round(4)) 
                +'\nHigher bound -->\n' +str(higher_bound.transpose().squeeze().round(4)))    
            return self.decide_bounds(lower_bound, higher_bound)
        else:
...
class envinit(randomize):
    ...
    def __init__(self,
        ...
        self._stiffeff = 0.0 # DIFFERENT DAMPING VALUES
        #self._stiffeff = 10.0 # DIFFERENT STIFFNESS VALUES
        self._stiffpos = 800.0
        #self._dampeff =  0.0 # DIFFERENT DAMPING VALUES
        self._dampeff = 10.0 # DIFFERENT DAMPING VALUES
        self._damppos = 40.0
        ...
        print("Loading asset '%s' from '%s'" % (_franka_asset_file, _asset_root))
    ...
    def create_envs(self):
        ...
        """
        print("Creating %d environments" % self.args.num_envs)
        ...
        for i in range(self.args.num_envs):
            ...
            if self.args.random_initial_velocities:
                self.dof_state["vel"][0] =  0
                self.dof_state["vel"][1:7] = 0
        ...
        print(f"Exerpt rigid_props of a single env:\n{rigid_body_prop[1].mass}\n"
              f"{rigid_body_prop[1].com}\n"
              f"{rigid_body_prop[1].inertia.x}\n"
              f"{rigid_body_prop[1].inertia.y}\n"
              f"{rigid_body_prop[1].inertia.z}")
